...
export default async function run(page) {
  ...
  await page.goto(`${BASE}/logout`); await page.goto(`${BASE}/login`); await page.locator("#email").fill("browser_step14_b@example.com"); await page.locator("#password").fill("Pass12345"); await page.locator('button[type="submit"]').click(); await page.waitForLoadState("domcontentloaded"); await page.goto(`${BASE}/notifications`); check("student B cannot see student A notifications", !(await page.innerText("body")).includes("Database Notes has been approved")); await page.goto(`${BASE}/logout`); await page.goto(`${BASE}/notifications`); check("notifications protected", page.url().includes("/login"));
